...
protected:
  ...
  public:
    ...
    v_io_size read(void *buffer, v_buff_size count,
                   async::Action &action) override;
  };
...
public:
   ...
   * @return - `std::shared_ptr` to Resources.
   */
  static std::shared_ptr<Resources>
  streamResources(const oatpp::String &resDir) {
   ...
   */
  virtual oatpp::String getResource(const oatpp::String &filename);

  /**
   * Get streamed resource by filename.
   * @param filename - name of the resource file.
   * @return - `std::shared_ptr` to &id:oatpp::data::stream::ReadCallback;
   * containing resource binary data stream."
   */
  virtual std::shared_ptr<ReadCallback>
  getResourceStream(const oatpp::String &filename);

  /**
   * Returns true if this is a streaming ressource instance.
   * @return
   */
  virtual bool isStreaming() { return m_streaming; }
   ...
   * @return a MIME type
   */
  virtual std::string getMimeType(const std::string &filename) const;
};
